...
set(figureHandle, 'WindowButtonDownFcn',{@myCallback, app});
disp('here');
...
if ~sum( mydata.pos<1 | mydata.pos>size(mydata.img) )
    set(app.UIFigure,'UserData',mydata);
    showimages(app)
end
...
% h(1)=subplot(2,2,1); 
imagesc(app.MRI_UIAxes1, squeeze(mydata.img(:,mydata.pos(2),:))'); 
d(1,:)=[1 3];
